...
function logTrace(type) {
    if (!window.X) {
        return;
    }

    if (type === "SET") {
        // "SET", trace.targetPath, "TO", value, "ON", object, "BECAUSE", trace.sourcePath
        var targetPath = arguments[1];
        var object = arguments[5];
        var sourcePath = arguments[7];
        if (object.constructor) {
            var name = object.constructor.name;
        }
        //if (!(name in {Iteration: 1, RangeController: 1, Repetition: 1, TreeControllerNode: 1, TreeController: 1, Indent: 1})) {
        console.log(name, targetPath, " <- ", sourcePath);
        //}
    } else if (type === "SWAPPING") {
        var minus = arguments[1];
        var plus = arguments[3];
        var index = arguments[5];
        var targetPath = arguments[7];
        // logTrace("SWAPPING", minus, "FOR", plus, "AT", index, "ON", trace.targetPath);
        console.log("swapping ", "+"+minus.length, "-"+plus.length, "@"+index, " " + targetPath)
    } else {
        console.log(type);
    }
}
...
function bindProperty(object, key, observeValue, source, descriptor, trace) {
    return observeValue(autoCancelPrevious(function replaceBoundPropertyValue(value) {
        ...
        try {
            ...
            trace && logTrace("SET", trace.targetPath, "TO", value, "ON", object, "BECAUSE", trace.sourcePath, getStackTrace());
...
function bindGet(collection, key, observeValue, source, descriptor, trace) {
    return observeValue(autoCancelPrevious(function replaceValue(value) {
        ...
        try {
            descriptor.isActive = true;
            trace && logTrace("SET FOR KEY", key, "TO", value, "ON", collection, "BECAUSE", trace.sourcePath, getStackTrace());
                ...
                return observeHas(autoCancelPrevious(function changeWhetherSetHas(has) {
                    ...
                    if (has) { // should be in set
                        if (!(set.has || set.contains).call(set, value)) {
                            trace && logTrace("ADD", value, "TO", trace.targetPath, "BECAUSE", trace.sourcePath, getStackTrace());
                            set.add(value);
                        }
                    } else { // should not be in set
                        while ((set.has || set.contains).call(set, value)) {
                            trace && logTrace("REMOVE", value, "FROM", trace.targetPath, "BECAUSE", trace.sourcePath, getStackTrace());
...
function makeEqualityBinder(bindLeft, observeRight) {
    return function bindEquals(observeEquals, source, target, descriptor, trace) {
        ...
        return observeEquals(autoCancelPrevious(function changeWhetherEquals(equals) {
            if (equals) {
                trace && logTrace("BIND", trace.targetPath, "TO", trace.sourcePath, getStackTrace());
                // a <-> b
                var cancel = bindLeft(observeRight, source, source, descriptor, trace);
                return function cancelEqualityBinding() {
                    trace && logTrace("UNBIND", trace.targetPath, "FROM", trace.sourcePath, getStackTrace());
                };
            }
        }), target);
    };
            ...
            return observeSource(autoCancelPrevious(function replaceRangeContentSource(source) {
                ...
                function rangeContentSourceRangeChange(plus, minus, index) {
                    if (isActive(target))
                        return;
                    if (trace) {
                        logTrace("SWAPPING", minus, "FOR", plus, "AT", index, "ON", trace.targetPath, getStackTrace());
            ...
            return observeSource(autoCancelPrevious(function replaceMapContentBindingSource(source) {
                ...
                function mapChange(value, key) {
                    ...
                    try {
                        ...
                        if (value === undefined) {
                            if (trace) {
                                trace && logTrace("DELETED", trace.targetPath, "FOR KEY", key, "ON", target, "BECAUSE", trace.sourcePath, getStackTrace());
                            }
                            ...
                            if (trace) {
                                trace && logTrace("SET", trace.targetPath, "FOR KEY", key, "TO", value, "ON", target, "BECAUSE", trace.sourcePath, getStackTrace());
                            }
